    ...
    try:
        ...
        if not bead_label:
            if current_idx <= 1:
                ...
                lbl = "● DUŻY PACIOREK — Ojcze Nasz"
            elif current_idx in (3, 4, 5):
                lbl = f"Paciorek wstępny {current_idx - 2} z 3 — Zdrowaś Maryjo"
            elif current_idx == 6:
                lbl = "● DUŻY PACIOREK — Chwała Ojcu / Zapowiedź Tajemnicy"
            elif current_idx <= 16:
                decade_bead = (current_idx - 6)
                lbl = f"Dziesiątek: Paciorek {decade_bead} z 10 (Zdrowaś Maryjo)"
            elif current_idx == 17:
                lbl = "● DUŻY PACIOREK — Chwała Ojcu / O mój Jezu"
            else:
